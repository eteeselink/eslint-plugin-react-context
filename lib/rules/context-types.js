...
var DIRECT_PROPS_REGEX = /^context\s*(\.|\[)/;
...
module.exports = Components.detect(function(context, components, utils) {
  ...
  var MISSING_MESSAGE = '\'{{name}}\' is missing in context validation';
  ...
  function isPropTypesUsage(node) {
    var isClassUsage = (
      ...
      node.object.type === 'ThisExpression' && node.property.name === 'context'
    );
    var isStatelessFunctionUsage = node.object.name === 'context';
  ...
  function isAnnotatedClassPropsDeclaration(node) {
    if (node && node.type === 'ClassProperty') {
      ...
      if (
        node.typeAnnotation && (
          tokens[0].value === 'context' ||
          (tokens[1] && tokens[1].value === 'context')
        )
      ) {
        return true;
      }
  ...
  function isAnnotatedFunctionPropsDeclaration(node) {
    if (node && node.params && node.params.length) {
      ...
      var isProps = tokens[0].value === 'context' || (tokens[1] && tokens[1].value === 'context');
      ...
      case 'VariableDeclarator':
        for (var i = 0, j = node.id.properties.length; i < j; i++) {
          ...
          var thisDestructuring = (
            (node.id.properties[i].key.name === 'context' || node.id.properties[i].key.value === 'context') &&
            node.id.properties[i].value.type === 'ObjectPattern'
          );
          // let {firstname} = props
          var statelessDestructuring = node.init.name === 'context' && utils.getParentStatelessComponent();
    ...
    switch (type) {
      ...
      case 'destructuring':
        for (var k = 0, l = properties.length; k < l; k++) {
          ...
          allNames = [];
          while (currentNode.property && currentNode.property.name !== 'context') {
  ...
  return {
    ...
    VariableDeclarator: function(node) {
      ...
      // let {firstname} = props
      var statelessDestructuring = destructuring && node.init.name === 'context' && utils.getParentStatelessComponent();
